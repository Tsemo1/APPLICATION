...
//import assogym.view.evenement.ViewGestionPoints;
...
public class ViewUtilisateurCombo2 extends ControllerAbstract {
...
//		list1 = (ArrayList<Integer>) daoUtilisateur.selectAnnee();
//		list1.add(0);
//		ObservableList<Integer> annee = FXCollections.observableArrayList(list1);
//		cmbAnnee.setItems(annee);
...
//		validator.addRule(txfNom, "Ce pseudo est déjà utilisé", modelUtilisateur::verifierUnicitePseudo  );
		...
		validator.addRuleMaxValue(dateN, LocalDate.now());
...
//		afficherPhoto(txfPhoto.getText());
	...
	private void doAjouter() {
		...
		if (!(selectedDate == null || selectedDate.isBefore(currentDate))) {
			...
			// invalide
			managerGui.showDialogError("La date sélectionnée est posterieure à la date du jour.");
//		        return;
		} else {
	...
	@FXML
	void doGererPoints(ActionEvent event) {
		//managerGui.showView(ViewGestionPoints.);
	...
	void user(ActionEvent event) {
		managerGui.showView(ViewUtilisateurCombo2.class);
	}
