...
public class DaoRole {
	...
	public void mettreAJourPourParti( Parti compte ) {
		...
		try {
			...
			stmt = cn.prepareStatement( sql, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE );
			stmt.setObject( 1, compte.getEmail() );
			rs = stmt.executeQuery();
			...
			for ( var item : set ) {
				rs.moveToInsertRow();
				rs.updateObject( "email", compte.getEmail() );
				rs.updateObject( "role", item );
				rs.insertRow();
			}
	...
	public List<String> listerPourParti( Parti compte ) {
		...
		try {
			...
			sql = "SELECT * FROM role WHERE email = ? ORDER BY role";
			stmt = cn.prepareStatement(sql);
			stmt.setObject( 1, compte.getEmail() );
			rs = stmt.executeQuery();
